...
// Adjacent actions can differ by floating-point rounding (for example,
// 34.55 versus 34.550000000000004). Treat those as the same handoff so the
// next claimant starts from the rail the previous action visibly settled.
export function avatarRailAt(time, peopleCount, actions = BOARD_ACTIONS) {
  const reached = boundary => time >= boundary - 1e-8;
  return actions.reduce((rail, event) => {
    if (event.to === 'claimed' && reached(event.start + BOARD_PHASES.approach))
      return rail.filter(person => person !== event.person);
    if (event.to === 'closed' && reached(event.end) && !rail.includes(event.person))
...
export function demoColumnMissions(missions, status) {
  ...
  return status === 'open' ? column.sort((a, b) => (a.openOrder ?? 0) - (b.openOrder ?? 0) || Number(Boolean(b.boosted)) - Number(Boolean(a.boosted))) : column.sort((a, b) => (b.stopped ?? b.started ?? 0) - (a.stopped ?? a.started ?? 0));
}

export function automationState(time, templates, people, actions = BOARD_ACTIONS) {
  const action = actions.find(a => time >= a.start && time < a.end) ?? null;
  ...
  const missions = templates.map((mission, i) => {
    const claim = actions.find(a => a.mission === i && a.to === 'claimed');
    const close = actions.find(a => a.mission === i && a.to === 'closed');
