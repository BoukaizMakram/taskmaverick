...
export function automationState(time, templates, people) {
  ...
  const missions = templates.map((mission, i) => {
    ...
    const status = close && time >= close.confirm ? 'closed' : claim && time >= claim.confirm ? 'claimed' : 'open';
    ...
    const timers = missionTimers(
      ...
      claim && status !== 'open' ? simulatedTime(claim.confirm) : undefined,
    ...
    return {
      ...
      showPerformerName: time >= SCENE['recognition-stamped'].start,
    };
